...
I2CEepromBlockDevice::I2CEepromBlockDevice(PinName sda, PinName scl,
        uint8_t address, bd_size_t block_size,
        bd_count_t block_count, int bus_speed)
...
bool I2CEepromBlockDevice::_check_space(bd_block_t block, bd_count_t count) {
    ...
    // extend past.
    return (block < _block_count && (block + count) < _block_count);
}
