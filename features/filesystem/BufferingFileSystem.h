 ...
 */
#ifndef MBED_BUFFERINGFILESYSTEM_H
#define MBED_BUFFERINGFILESYSTEM_H

#include "FileSystem.h"
#include "BlockDevice.h"

#ifndef MBED_BUFFERINGFS_DEFAULT_SIZE
#define MBED_BUFFERINGFS_DEFAULT_SIZE 32
#endif


/**
 * BufferingFileSystem, a file system which manages a RAM backed buffer
 * for each opened file.
 *
 * This enables all of the POSIX functions, such as seek and partial writes,
 * with one-time read and write operations during open and close. The cost is
 * a buffer the size of the file allocated in each open call.
 */
class BufferingFileSystem : public mbed::FileSystem {
public:
    /** Lifetime of the BufferingFileSystem
     *
     *  @param name         Name to add filesystem to tree as
     *  @param fs           Underlying filesystem to buffer
     */
    BufferingFileSystem(const char *name=NULL, mbed::FileSystem *fs=NULL);
    virtual ~BufferingFileSystem();
