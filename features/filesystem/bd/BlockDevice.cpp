...
// Defaults to program size
bd_size_t BlockDevice::read_size()
{
    return program_size();
}

// Defaults to erase size
bd_size_t BlockDevice::program_size()
{
    return erase_size();
}


// Convenience functions for implementations
bool BlockDevice::is_aligned(uint64_t x, uint64_t alignment)
{
