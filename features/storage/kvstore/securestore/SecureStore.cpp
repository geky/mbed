...
static const uint32_t security_flags = KVStore::AUTHENTICATE_FLAG | KVStore::ENCRYPT_FLAG | KVStore::ROLLBACK_PROTECT_FLAG;
                           ...
                           uint32_t create_flags)
{
    ...
    // RBP requires authentication
    if ((create_flags & ROLLBACK_PROTECT_FLAG) && !(create_flags & AUTHENTICATE_FLAG)) {
        return MBED_ERROR_INVALID_ARGUMENT;
    }
    ...
    *handle = static_cast<set_handle_t>(_inc_set_handle);
    ...
    if (ret == MBED_SUCCESS) {
        // Must not remove Rollback protect flag
        if (!(create_flags & ROLLBACK_PROTECT_FLAG) && (ih->metadata.create_flags & ROLLBACK_PROTECT_FLAG)) {
    ...
    if (create_flags & AUTHENTICATE_FLAG) {
    ...
    if (create_flags & ROLLBACK_PROTECT_FLAG) {
...
int SecureStore::set_add_data(set_handle_t handle, const void *value_data, size_t data_size)
{
    ...
    while (data_size) {
        ...
        if (ih->metadata.create_flags & ENCRYPT_FLAG) {
        ...
        if (ih->metadata.create_flags & AUTHENTICATE_FLAG) {
...
int SecureStore::set_finalize(set_handle_t handle)
{
    ...
    if (_rbp_kv && (ih->metadata.create_flags & ROLLBACK_PROTECT_FLAG)) {
...
int SecureStore::remove(const char *key)
{
    ...
    if (_rbp_kv && (info.flags & ROLLBACK_PROTECT_FLAG)) {
                        ...
                        size_t offset, info_t *info)
{
    ...
    if (!_rbp_kv) {
        create_flags &= ~ROLLBACK_PROTECT_FLAG;
    }

    // Another potential attack case - key hasn't got the RBP flag set, but exists in the RBP KV
    if (rbp_key_exists && !(create_flags & ROLLBACK_PROTECT_FLAG)) {
        ret = MBED_ERROR_RBP_AUTHENTICATION_FAILED;
        goto end;
    }
