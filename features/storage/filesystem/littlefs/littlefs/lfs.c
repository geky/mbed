...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    // TODO HACK, we know we fit in two blocks
...
static int lfs_dir_fetch(lfs_t *lfs, lfs_mdir_t *dir) {
    ...
    return lfs_dir_fetchmatch(lfs, dir, -1, 0, NULL, NULL, NULL);
}
...
//    dir->tail[0] = (const lfs_block_t[2]){0, 1}[0];
...
int lfs_get(lfs_t *lfs, const char *key, void *buffer, lfs_size_t size) {
    ...
    lfs_mdir_t dir;
    uint16_t id;
    lfs_size_t keylen = strlen(key);
    lfs_stag_t tag = lfs_dir_fetchmatch(lfs, &dir,
            LFS_MKTAG(0x780, 0, 0),
            LFS_MKTAG(LFS_TYPE_NAME, 0, keylen), &id,
            lfs_dir_find_match, &(struct lfs_dir_find_match){
                lfs, key, keylen});
    if (tag < 0 && !(tag == LFS_ERR_NOENT && id != 0x3ff)) {
        return tag;
    }

    tag = lfs_dir_get(lfs, &dir,
            LFS_MKTAG(0x7ff, 0x3ff, 0),
            LFS_MKTAG(LFS_TYPE_INLINESTRUCT, id, size), buffer);
    if (tag < 0) {
        return tag;
    }
...
int lfs_set(lfs_t *lfs, const char *key, const void *buffer, lfs_size_t size) {
    ...
    int err = lfs_dir_commit(lfs, &dir, LFS_MKATTRS(
            ...
            {tag == LFS_ERR_NOENT
                ? LFS_MKTAG(LFS_TYPE_REG, id, keylen)
                : LFS_MKTAG(LFS_FROM_NOOP, 0, 0), key},
            {LFS_MKTAG(LFS_TYPE_INLINESTRUCT, id, size), buffer}));
    if (err) {
        return err;
    }
...
//        }
        ...
        // the revision may be valid
        int err = lfs_bd_read(lfs,
                NULL, &lfs->rcache, sizeof(root.rev),
                root.pair[0], 0, &root.rev, sizeof(root.rev));
        if (err) {
            return err;
        }

        root.rev = lfs_fromle32(root.rev);
        if (err && err != LFS_ERR_CORRUPT) {
            return err;
        }

        // set defaults
        root.off = sizeof(root.rev);
        root.etag = 0xffffffff;
        root.count = 0;
//        root.tail[0] = 0xffffffff;
//        root.tail[1] = 0xffffffff;
        root.erased = false;
//        root.split = false;
