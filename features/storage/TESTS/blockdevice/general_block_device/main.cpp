...
void test_get_erase_value()
{
    ...
    bd_size_t data_buf_size = block_device->get_erase_size();
    TEST_ASSERT(data_buf_size > 0);
    bd_addr_t start_address = rand();       // low 32 bytes
    start_address += (uint64_t)rand() << 8; // high 32 bytes
    start_address *= block_device->get_erase_size(); // aligned with erase_size
    start_address %= block_device->size() - data_buf_size; //  fit all data
...
void test_contiguous_erase_write_read()
{
    ...
    int contiguous_write_read_blocks_per_region = write_read_buf_size / program_size; // 2 is minimum to test contiguous write
    ...
    bd_addr_t start_address = rand();      // low 32 bytes
    start_address += (uint64_t)rand() << 8;// high 32 bytes
    start_address *= erase_size; // aligned with erase_block
    start_address %= block_device->size() - contiguous_erase_size; //  Must fit all data
