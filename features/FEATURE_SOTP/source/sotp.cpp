...
int SOTP::flash_read_area(uint8_t area, uint32_t offset, uint32_t len_bytes, uint32_t *buf)
{
...
// buf           - [IN]   Data buffer.
// Return        : 0 on success. Error code otherwise.
int SOTP::flash_write_area(uint8_t area, uint32_t offset, uint32_t len_bytes, const uint32_t *buf)
{
...
// area          - [IN]   Flash area.
// Return        : 0 on success. Error code otherwise.
int SOTP::flash_erase_area(uint8_t area)
{
...
// Return        : 0 on success. Error code otherwise.
int SOTP::calc_empty_space(uint8_t area, uint32_t *offset)
{
    uint32_t buf[32];
    uint8_t *chbuf;
    uint32_t i, j;
...
sotp_result_e SOTP::read_record(uint8_t area, uint32_t offset, uint16_t buf_len_bytes, uint32_t *buf,
...
// Return        : SOTP_SUCCESS on success. Error code otherwise.
sotp_result_e SOTP::write_record(uint8_t area, uint32_t offset, uint8_t type, uint8_t flags,
...
// next_offset   - [Out]  offset of next record.
// Return        : SOTP_SUCCESS on success. Error code otherwise.
sotp_result_e SOTP::write_master_record(uint8_t area, uint16_t version, uint32_t *next_offset)
{
...
// Return        : SOTP_SUCCESS on success. Error code otherwise.
sotp_result_e SOTP::copy_record(uint8_t from_area, uint32_t from_offset, uint32_t to_offset,
...
// Return      : SOTP_SUCCESS on success. Error code otherwise.
sotp_result_e SOTP::garbage_collection(uint8_t type, uint16_t buf_len_bytes, const uint32_t *buf)
{
    ...
    if (flash_erase_area(1 - active_area))
        return SOTP_WRITE_ERROR;
...
// validate_only    - [IN]   Just validate (don't return user data).
// Return      : SOTP_SUCCESS on success. Error code otherwise.
sotp_result_e SOTP::do_get(uint8_t type, uint16_t buf_len_bytes, uint32_t *buf, uint16_t *actual_len_bytes,
                           int validate_only)
{
...
sotp_result_e SOTP::get(uint8_t type, uint16_t buf_len_bytes, uint32_t *buf, uint16_t *actual_len_bytes)
{
    return do_get(type, buf_len_bytes, buf, actual_len_bytes, 0);
}

sotp_result_e SOTP::get_item_size(uint8_t type, uint16_t *actual_len_bytes)
{
    return do_get(type, 0, NULL, actual_len_bytes, 1);
}

sotp_result_e SOTP::do_set(uint8_t type, uint16_t buf_len_bytes, const uint32_t *buf, uint8_t flags)
{
    sotp_result_e ret = SOTP_SUCCESS;
    ...
    if (new_free_space >= flash_area_params[active_area].size) {
        ...
        if (record_offset < flash_area_params[active_area].size) {
            ...
            ret = garbage_collection((uint8_t)type, buf_len_bytes, buf);
...
sotp_result_e SOTP::remove(uint8_t type)
{
    return do_set(type, 0, NULL, DELETE_ITEM_FLAG);
}

#endif

sotp_result_e SOTP::init()
{
    uint8_t area;
    uint8_t type;
    uint8_t flags;
    ...
    area_state_e area_state[SOTP_INT_FLASH_NUM_AREAS];
    uint32_t free_space_offset_of_area[SOTP_INT_FLASH_NUM_AREAS];
    uint16_t versions[SOTP_INT_FLASH_NUM_AREAS];
    uint32_t next_offset;
    master_record_data_t master_rec;
    ...
    if ((area_state[0] == AREA_STATE_VALID) && (area_state[1] == AREA_STATE_VALID)) {
        ...
        os_ret = flash_erase_area(1 - active_area);
        if (os_ret) {
            PR_ERR("sotp_init: sotp_flash_erase_area failed with err code 0x%lx\n",
                    (unsigned long) os_ret);
            ret = SOTP_WRITE_ERROR;
    ...
    while (free_space_offset < free_space_offset_of_area[active_area]) {
        ...
        if (!valid) {
            ret = garbage_collection(SOTP_NO_TYPE, 0, NULL);
            break;
        }
        if (flags && DELETE_ITEM_FLAG)
            offset_by_type[type] = 0;
...
sotp_result_e SOTP::force_garbage_collection(void)
{
    sotp_result_e ret;
...
sotp_result_e SOTP::probe(uint8_t type, uint16_t buf_len_bytes, uint32_t *buf, uint16_t *actual_len_bytes)
{
